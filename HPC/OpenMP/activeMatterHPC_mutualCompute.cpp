...
void computeActiveMatter(generalPara_t gPara, activePara_t aPara, arrayPtr& posX, arrayPtr& posY, arrayPtr& theta){
    ...
    bool* mutualCompute(new bool[gPara.birdNum * gPara.birdNum / 2]);

    arrayPtr tempSx(new float[gPara.birdNum]), tempSy(new float[gPara.birdNum]);
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp for
                for(int bird=0; bird < gPara.birdNum; bird++){ //for each bird
                    ...
                    float& sx = tempSx[bird], &sy = tempSy[bird]; 

                    sx = 0;
                    sy = 0;

                    for(int i=0; i < gPara.birdNum / 2; i++){ //observe half of birds after self, birdNum / 2

                        mutualCompute[bird * gPara.birdNum / 2 + i] = false; //not in observation

                        int oBird=bird + 1 + i;
                        if(oBird >= gPara.birdNum)oBird = oBird % gPara.birdNum; 
                        ...
                        if((xDiffAbs < inscribedSquareSideLengthHalf) && 
                            ...
                            sx += cos(theta[oBird]);
                            sy += sin(theta[oBird]);
                            mutualCompute[bird * gPara.birdNum / 2 + i] = true;
                        }else{
            ...
            #pragma omp for
                for(int bird=0; bird < gPara.birdNum; bird++){ //for each bird
                    ...
                    for(int i=0; i < gPara.birdNum / 2 - 1; i++){ //birdNum / 2 -1

                        int oBird = i + bird + 1 + gPara.birdNum / 2;
                        int relativeIndex = gPara.birdNum / 2 - 2 - i;
                        if(oBird >= gPara.birdNum)oBird = oBird % gPara.birdNum; //absolute index of other bird

                        if(mutualCompute[oBird * gPara.birdNum / 2 + relativeIndex] == true){
                            sx += cos(theta[oBird]);
                            sy += sin(theta[oBird]);
                        }
